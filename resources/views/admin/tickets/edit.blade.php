...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ticket.title_singular') }}: ID {{ $ticket->id }} - Propietario : {{ $ticket->author_name }}  - Email :{{ $ticket->author_email }}
    </div>
    ...
    <div class="card-body">
        <form action="{{ route("admin.tickets.update", [$ticket->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                ...
                <input type="text" readonly="readonly" id="title" name="title" class="form-control" value="{{ old('title', isset($ticket) ? $ticket->title : '') }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group {{ $errors->has('content') ? 'has-error' : '' }}">
                ...
                <textarea id="content" name="content" readonly="readonly" class="form-control ">{{ old('content', isset($ticket) ? $ticket->content : '') }}</textarea>
            ...
            <div class="form-group {{ $errors->has('employee_id') ? 'has-error' : '' }}">
                <label for="employee">Asignar {{ trans('cruds.employee.title_singular') }}*</label>
                <select name="employee_id" id="employee_id" class="form-control select2" required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $employee->id }}" {{ (isset($ticket) && $ticket->employee ? $ticket->employee->id : old('employee_id')) == $id ? 'selected' : '' }}>{{ $employee->first_name }} {{ $employee->last_name }} - {{ $employee->rut }}</option>
                    @endforeach
                </select>
                @if($errors->has('category_id'))
                    <em class="invalid-feedback">
